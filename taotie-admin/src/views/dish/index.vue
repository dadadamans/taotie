<template>
  <div class="page">
    ...
    <div class="toolbar">
      ...
      <el-input v-model="searchName" placeholder="菜品名称" clearable style="width: 180px" @keyup.enter="handleSearch" />
      ...
      <div class="right">
        ...
        <el-button type="primary" @click="goAdd">+ 新增菜品</el-button>
      </div>
    </div>

    <div class="table-card">
      <el-table
        :data="list" v-loading="loading" stripe border
      ...
      >
        ...
        <el-table-column label="图片" width="70">
          <template #default="{ row }">
            <el-image
              v-if="row.image" :src="row.image" style="width: 44px; height: 44px; border-radius: 6px" fit="cover"
            ><template #error><span class="img-placeholder">无图</span></template></el-image>
        ...
        <el-table-column label="操作" width="150">
          <template #default="{ row }">
            <el-space>
              ...
              <el-button
                link :type="row.status === 1 ? 'warning' : 'success'"
                @click="handleStatusChange(row)"
              >{{ row.status === 1 ? '停售' : '起售' }}</el-button>
            </el-space>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-empty v-if="!loading && list.length === 0" description="暂无菜品数据" />
    </div>
    ...
    <ConfirmDialog
      ...
      type="warning"
      title="确认批量删除？"
      :description="`将删除 ${selectedIds.length} 个菜品，不可恢复`"
      confirm-text="确认删除"
    ...
    <ConfirmDialog
      ...
      type="warning"
      :title="`确认${statusConfirmRow?.status === 1 ? '停售' : '起售'}该菜品？`"
      confirm-text="确认"
...
import { getDishPage, setDishStatus, deleteDish, getCategoryList } from '@/api/shop'
...
async function confirmStatusChange() {
  ...
  const res = await setDishStatus(newStatus, statusConfirmRow.value.id)
  if (res.code === 1) {
...
function goEdit(id: number) { router.push(`/dish/form?id=${id}`) }
function goAdd() { router.push('/dish/form') }
...
.page { padding: 20px; background: #f5f7fb; min-height: 100vh; }
.header h2 { margin: 0; }
.header p { margin: 4px 0 16px; color: #888; }
.toolbar {
  display: flex; gap: 10px; align-items: center; background: #fff;
  padding: 12px; border-radius: 10px; margin-bottom: 16px; flex-wrap: wrap;
}
.right { margin-left: auto; display: flex; gap: 8px; }
.table-card { background: #fff; padding: 10px; border-radius: 10px; }
.pagination { margin-top: 12px; display: flex; justify-content: flex-end; }
.img-placeholder {
  display: inline-block; width: 44px; height: 44px; line-height: 44px;
  text-align: center; background: #f3f4f6; border-radius: 6px;
